    ...
    return {
        ...
        'csp-production': {
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: () => {
                        const html = fs.readFileSync('build/web-audio-conference-2019/index.html', 'utf-8');
        ...
        'manifest': {
            ...
            options: {
                patterns: [ {
                    match: /assets\/(?<filename>[\d\-a-z]+)\.(?<extension>ico|jpg|png)/g,
                    replacement: (_, filename, extension) => grunt.file.expand({ cwd: 'build/web-audio-conference-2019', ext: extension }, `assets/${ filename }.*`)[0]
                }, {
                    ...
                    match: /"\/web-audio-conference-2019(?<filename>\/scripts\/main(?:-es(?:2015|5))?.[\da-z]+.js)":\s"[\da-z]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-conference-2019${ filename }": "${ computeHashOfFile(`build/web-audio-conference-2019${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                ...
                patterns: [ {
                    ...
                    replacement: (match, filename, moduleAttribute) => {
                        if (moduleAttribute === undefined) {
                            return `<script>${ fs.readFileSync(`build/web-audio-conference-2019/${ filename }`) }</script>`;
                        }

                        return `<script${ moduleAttribute }>${ fs.readFileSync(`build/web-audio-conference-2019/${ filename }`) }</script>`;
                    }
                } ]
            }
        },
            ...
            options: {
                patterns: [ {
                    ...
                    replacement: (match, filename) => {
                        const hash = `sha384-${ computeHashOfFile(`build/web-audio-conference-2019/styles/${ filename }`, 'sha384', 'base64') }`;
